...
public class HTCommand implements CommandExecutor {
    ...
    public HTCommand() {
        arguments.put("forcestart", (player, args) -> HackerTools.getPlugin().getMinigameManager().force());
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length == 0) {
            HackerTools.getPlugin().getChatManager().sendBorderedMessage(sender, "■", false, "§c§l" + Placeholders.PLUGINNAME + "§7 Plugin was made for spigot minigame developers by §c§l" + Placeholders.PLUGINAUTHOR + "§7.");
            return true;
        }
        ...
        if (Permissions.hasPermission(player, "ht.command." + command.toLowerCase(), true)) {
    ...
    private void sendHelp(Player player) {
        ...
        lines.add("/ht restart - §arestarts the server");
        lines.add("/ht forcestart - §aforces the game to start");
        lines.add("/ht speed (number) - §asets your walk and fly speed");
